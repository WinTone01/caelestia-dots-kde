...
if [[ "${CAELESTIA_SETUP_RUNNING:-0}" == "0" ]]; then
    ...
    if command -v pacman >/dev/null; then
        ...
        sudo pacman -S --needed qt6-wayland kglobalaccel kglobalacceld --noconfirm || warn "qt6-wayland install failed..."
        ...
        sudo apt-get update && sudo apt-get install -y qt6-wayland qt6-wayland-dev libkf6globalaccel-dev libkf6windowsystem-dev qt6-base-private-dev || warn "apt install failed..."
    fi
